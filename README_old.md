![Cloudflare Image Uploader Screenshot](https://imagedelivery.net/gaLGizR3kCgx5yRLtiRIOw/3e6c9eae-a4bb-45f2-da37-f99ac9be3900/w=1200?format=webp)


# Photarium

An elegant, modern web application for uploading and managing images using Cloudflare Images. Perfect for hosting images that you need to use in email blasts, websites, or any other online content.
...
- **A Cloudflare account** with Cloudflare Images enabled
  - Don't have one yet? [Create a free Cloudflare account](https://dash.cloudflare.com/sign-up)
  - Then [enable Cloudflare Images](https://dash.cloudflare.com/?to=/:account/images/getting-started) in your dashboard (free tier includes 100k images/month)
- **Optional**: OpenAI API key (for AI-powered ALT text generation)
...
IMAGE_NAMESPACE=your_app_namespace
NEXT_PUBLIC_IMAGE_NAMESPACE=your_app_namespace
```

`IMAGE_NAMESPACE` scopes uploads, duplicate detection, and `/api/images` to a logical app namespace. `NEXT_PUBLIC_IMAGE_NAMESPACE` seeds the UI with the same value; you can override it in the app’s namespace settings.

### Backfill existing images with a namespace

To tag existing Cloudflare images that don’t have a namespace yet, run:
...
The script reads `.env.local` for `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_API_TOKEN`, and `IMAGE_NAMESPACE` (unless you pass `--namespace`).
