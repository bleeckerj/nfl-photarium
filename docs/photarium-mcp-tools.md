...
- `photarium_record_creative_brief_result` requires an uploaded generated child, verifies its source-family parentage, derives or validates dimensions and actual aspect ratio, then generates and saves description and alt text for that child. Its response includes a completion object and `metadataEnrichment.status` as `completed`, `partial`, or `failed`; a prepared plan or provider handoff is not completion.
- Creative-brief generation does not create or accept operational tags. Provider, relationship, derivation, and workflow information remain in provenance and derivation fields; image tags are reserved for explicit semantic tagging operations.
...
Every actual Codex-generated image must be uploaded to Photarium and verified before the workflow is complete. A local file, provider handoff, or missing hosted child URL is an incomplete result.
