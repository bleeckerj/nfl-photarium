...
export function decodeUrlComponentSafe(value) {
    try {
...
export function extractFilenameFromSearchParams(params) {
...
export function extractFilenameFromQueryBlob(value) {
...
export function camelizeUploadStem(value) {
    const tokens = value
...
export function extensionFromMimeType(value) {
    if (!value)
        return undefined;
...
export function extensionFromFilename(value) {
    const match = value.match(/\.[a-z0-9]{2,5}$/i);
    return match ? match[0].toLowerCase() : undefined;
}
export function withExtension(stem, extension) {
    if (!extension)
        return stem;
...
export function extractUploadFilenameFromUrl(url, mimeType) {
...
export function looksLikeTransportFilename(filename) {
