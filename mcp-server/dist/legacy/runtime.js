...
async function runInstagramSingleUrlIngest(options) {
...
async function runDiscordRefreshAndIngest(options) {
...
function extractFilenameFromSearchParams(params) {
...
function extractFilenameFromQueryBlob(value) {
...
function extractUploadFilenameFromUrl(url, mimeType) {
...
function looksLikeTransportFilename(filename) {
    const lowered = filename.toLowerCase();
    const stem = lowered.replace(/\.[^.]+$/, '');
    if (stem.includes('view_filename='))
        return true;
    if (stem.includes('filename=') && stem.includes('&'))
        return true;
    if (stem === 'view' || stem === 'image' || stem === 'uploaded-image' || stem === 'remote-image')
        return true;
    if (/^comfyui[_-]?\d+_?$/.test(stem))
        return true;
    return /[=&]/.test(stem);
}
...
async function suggestSemanticDisplayNameFromUrl(url, hints = {}) {
    try {
...
function normalizeManualPrompt(value) {
    if (typeof value !== 'string')
        return undefined;
    const trimmed = value.trim();
...
export const LEGACY_TOOLS = [
    ...
    },
    {
        ...
        description: 'Recursively ingest local image/video files from a directory tree into a specific namespace via Photarium. Includes subdirectory path in descriptions and can optionally generate image display names/tags with AI.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Refresh local Discord image exports (newest channel content), then ingest those files into Photarium via fs:ingest. Use this when you need a fresh pull + upload pass.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Ingest a single Instagram post or reel URL through the existing Instagram CLI/browser profile flow and push any recovered video into Photarium. Defaults to namespace "ig-videos" and fallback owner username "darthjulian". Uses the same persistent profile directory as the local Instagram CLI unless overridden.',
        inputSchema: {
            type: 'object',
            properties: {
                url: {
...
export async function handleLegacyToolCall(name, args = {}) {
    try {
        switch (name) {
            case 'list_tools': {
                return {
                    content: [
                        {
                            type: 'text',
                            text: JSON.stringify({ tools: LEGACY_TOOLS }, null, 2),
                        },
                    ],
                };
            }
            ...
            case 'photarium_upload_file': {
                const { base64, filename, contentType, folder, tags, description, prompt, originalUrl, sourceUrl, sourcePath, namespace, parentId } = args;
            ...
            case 'photarium_upload_image': {
                const { base64, filename, contentType, folder, tags, description, prompt, originalUrl, sourceUrl, namespace, parentId, useExternalApi } = args;
            ...
            case 'photarium_upload_external_file': {
                const { base64, filename, contentType, folder, tags, description, prompt, originalUrl, sourceUrl, namespace, parentId } = args;
            ...
            case 'photarium_upload_from_path': {
                const { filePath, filename, folder, tags, description, prompt, originalUrl, sourceUrl, namespace, parentId } = args;
                try {
            ...
            case 'photarium_fs_ingest': {
                const { rootPath, namespace, apiBase, folder, tags, descriptionPrefix, includeFilename, includePathTags, aiMetadata, aiDisplayName, aiTags, tagCount, concurrency, throttleMs, limit, dryRun, verbose, } = args;
            ...
            case 'photarium_discord_refresh_and_ingest': {
            ...
            case 'photarium_instagram_ingest_single_url': {
