...
export const archiveHandlers: Record<string, RuntimeToolHandler> = {
  ...
  archive_search: async (args) => {
    const result = await archiveJson<{ results: Array<{ id: string }>; expandedTerms: string[] }>('/search', { method: 'POST', body: JSON.stringify(searchBody(args)) });
    const includePreviews = args.includePreviews === true;
    const content: Array<{ type: 'text'; text: string } | { type: 'image'; data: string; mimeType: string }> = [{ type: 'text', text: JSON.stringify(result, null, 2) }];
    if (includePreviews) {
