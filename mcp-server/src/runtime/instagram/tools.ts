...
export const instagramTools: Tool[] = [
  {
    ...
    description:
      'Open the existing Instagram auth flow using a persistent Chromium profile and validate the session for a username. This is interactive and should usually be run headful.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Ingest a single Instagram post or reel URL through the existing Instagram CLI/browser profile flow. Defaults to namespace "ig-videos", fallback owner username "darthjulian", and Cloudflare push enabled for backward compatibility.',
    inputSchema: {
      type: 'object',
      properties: {
        url: {
