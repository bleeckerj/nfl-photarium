...
import { instagramTools } from './instagram/tools.js';
...
import type { RuntimeToolHandler, RuntimeToolModule, RuntimeToolResult } from './types.js';
import { uploadHandlers } from './upload/handlers.js';
import { uploadTools } from './upload/tools.js';

const LIST_TOOLS_TOOL: Tool = {
  name: 'list_tools',
...
export const RUNTIME_TOOLS: Tool[] = [
  LIST_TOOLS_TOOL,
  ...runtimeModules.flatMap((runtimeModule) => runtimeModule.tools),
];

function buildHandlerMap(modules: RuntimeToolModule[]): Map<string, RuntimeToolHandler> {
...
export async function handleRuntimeToolCall(name: string, args: Record<string, unknown> = {}): Promise<RuntimeToolResult> {
  try {
