...
async function runDiscordRefreshAndIngest(options: {
  ...
  if (options.discordRepo) args.push('--discord-repo', options.discordRepo);
  if (options.imagesRoot) args.push('--images-root', options.imagesRoot);
  if (options.namespace) args.push('--namespace', options.namespace);
  if (options.visuallyNamespace) args.push('--visually-namespace', options.visuallyNamespace);
  if (options.autotraderNamespace) args.push('--autotrader-namespace', options.autotraderNamespace);
  if (options.apiBase) args.push('--api-base', options.apiBase);
  if (options.checkpointFile) args.push('--checkpoint-file', options.checkpointFile);
  if (options.tags && options.tags.length > 0) args.push('--tags', options.tags.join(','));
  if (options.appendImageTag) args.push('--append-image-tag', options.appendImageTag);
  if (options.descriptionPrefix) args.push('--description-prefix', options.descriptionPrefix);
  if (typeof options.tagCount === 'number') args.push('--tag-count', String(options.tagCount));
  if (typeof options.concurrency === 'number') args.push('--concurrency', String(options.concurrency));
  if (typeof options.throttleMs === 'number') args.push('--throttle-ms', String(options.throttleMs));
  if (options.noAiMetadata) args.push('--no-ai-metadata');
  if (options.includePathTags) args.push('--include-path-tags');
  if (options.includeFilename) args.push('--include-filename');
  if (options.hashCacheBackfillOnly) args.push('--hash-cache-backfill-only');
  if (options.reportCache) args.push('--report-cache');
  if (options.assumeUploaded) args.push('--assume-uploaded');
  if (options.skipDiscordRefresh) args.push('--skip-discord-refresh');
  if (options.skipIngest) args.push('--skip-ingest');
  if (options.dryRun) args.push('--dry-run');
  if (options.verbose) args.push('--verbose');
...
export const LEGACY_TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description:
      'Ingest a single Instagram post or reel URL through the existing Instagram CLI/browser profile flow and push any recovered video into Photarium. Defaults to namespace "ig-videos" and fallback owner username "darthjulian". Uses the same persistent profile directory as the local Instagram CLI unless overridden.',
    inputSchema: {
      type: 'object',
      properties: {
        url: {
...
export async function handleLegacyToolCall(name: string, args: Record<string, unknown> = {}) {
  try {
    switch (name) {
      case 'list_tools': {
        return {
